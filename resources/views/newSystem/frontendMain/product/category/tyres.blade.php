...
<div class="container mt-5">
    ...
    <div class="row">
        @foreach ($product_brands as $product_brand => $value)
            @php
                $product_models = DB::table('product_models')->where('subcategory_id',$value->id)
                                                             ->orderByRaw("FIELD(model, 'ENERGY XM2+','PRIMACY 4','PILOT SPORT 3','PILOT SPORT 4','PILOT SPORT 4 SUV','PILOT SPORT 5','LTX TRAIL','PRIMACY SUV+','AGILIS 3','XCD2','TA Drive','TA SUV','ADVANTAGE TOURING','TRAIL TERRAIN','All-Terrain KO2','MUD TERRAIN KM3') ASC")
                                                             ->where('status','เปิด')
                                                             ->get(); 
            @endphp
